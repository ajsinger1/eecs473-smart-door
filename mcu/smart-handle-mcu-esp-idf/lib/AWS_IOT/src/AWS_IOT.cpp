...
ExploreEmbedded invests substantial time and effort developing open source HW and SW tools, to support consider buying
the ExploreEmbedded boards.
...
void disconnectCallbackHandler(AWS_IoT_Client* pClient, void* data) {
  ...
  if (aws_iot_is_autoreconnect_enabled(pClient)) {
    ESP_LOGE(TAG, "Auto Reconnect is enabled, Reconnecting attempt will start now");
  } else {
    ESP_LOGE(TAG, "Auto Reconnect not enabled. Starting manual reconnect...");
...
int AWS_IOT::connect(const char* hostAddress, const char* clientID) {
  ...
  do {
    ...
    if (SUCCESS != rc) {
      ESP_LOGE(
          TAG, "Error(%d) connecting to %s:%d, \n\rTrying to reconnect", rc, mqttInitParams.pHostURL,
          mqttInitParams.port);
    }
...
int AWS_IOT::unsubscribe(const char* subTopic) {
  IoT_Error_t rc;

  ESP_LOGI(TAG, "Unsubscribing...");
  rc = aws_iot_mqtt_unsubscribe(&client, subTopic, strlen(subTopic));
  if (SUCCESS != rc) {
    ESP_LOGE(TAG, "Error unsubscribing : %d ", rc);
    return rc;
  }
  ESP_LOGI(TAG, "Unsubscribing... Successful");
...
void aws_iot_task(void* param) {
  ...
  while (1) {
    ...
    if (rc != SUCCESS) {
      Serial.println("IOT FUNCTION FAILED (" + String(rc) + ")");
    }
