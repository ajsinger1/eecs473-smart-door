...
NimBLEService* NimBLEServer::createService(const NimBLEUUID& uuid) {
  ...
  if (getServiceByUUID(uuid) != nullptr) {
    NIMBLE_LOGW(LOG_TAG, "Warning creating a duplicate service UUID: %s", std::string(uuid).c_str());
  }
 ...
 */
NimBLEExtAdvertising* NimBLEServer::getAdvertising() { return NimBLEDevice::getAdvertising(); } // getAdvertising
#endif
 ...
 * @return An advertising object.
 */
NimBLEAdvertising* NimBLEServer::getAdvertising() { return NimBLEDevice::getAdvertising(); } // getAdvertising
#endif
...
void NimBLEServer::start() {
  ...
  */
  ...
  for (auto& svc : m_svcVec) {
    if (svc->m_removed == 0) {
      Serial.println(String(((long)(&svc->getUUID().getNative()->u)), HEX));
 ...
 */
void NimBLEServer::advertiseOnDisconnect(bool aod) { m_advertiseOnDisconnect = aod; } // advertiseOnDisconnect
#endif
 ...
 */
size_t NimBLEServer::getConnectedCount() { return m_connectedPeersVec.size(); } // getConnectedCount

/**
 * @brief Get the vector of the connected client ID's.
 */
std::vector<uint16_t> NimBLEServer::getPeerDevices() { return m_connectedPeersVec; } // getPeerDevices
...
int NimBLEServer::handleGapEvent(struct ble_gap_event* event, void* arg) {
  ...
  case BLE_GAP_EVENT_DISCONNECT: {
    ...
    server->m_connectedPeersVec.erase(
        std::remove(
            server->m_connectedPeersVec.begin(), server->m_connectedPeersVec.end(), event->disconnect.conn.conn_handle),
 ...
 */
bool NimBLEServer::startAdvertising(uint8_t inst_id, int duration, int max_events) {
 ...
 */
bool NimBLEServer::stopAdvertising(uint8_t inst_id) { return getAdvertising()->stop(inst_id); } // stopAdvertising
#endif
 ...
 * retrieving the advertising object and invoking start upon it.
 */
bool NimBLEServer::startAdvertising() { return getAdvertising()->start(); } // startAdvertising
#endif
 ...
 * @return True if advertising stopped successfully.
 */
bool NimBLEServer::stopAdvertising() { return getAdvertising()->stop(); } // stopAdvertising

/**
 * @brief Get the MTU of the client.
 * @returns The client MTU or 0 if not found/connected.
 */
uint16_t NimBLEServer::getPeerMTU(uint16_t conn_id) { return ble_att_mtu(conn_id); } // getPeerMTU
...
void NimBLEServer::updateConnParams(
    uint16_t conn_handle, uint16_t minInterval, uint16_t maxInterval, uint16_t latency, uint16_t timeout) {
